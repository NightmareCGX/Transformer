...
def build_posenc(max_len: int, d_model: int, device: str | torch.device = None):
...
def causal_mask(L: int, device: str | torch.device = None, dtype: torch.dtype = torch.float32):
    matrix = torch.zeros((1, 1, L, L), device=device, dtype=dtype)
    matrix = matrix.masked_fill(
        torch.triu(torch.ones((1, 1, L, L), dtype=torch.bool, device=device), diagonal=1),
        float("-inf"),
    )
...
def padding_mask(key_padding: torch.Tensor, dtype: torch.dtype = torch.float32):
    ...
    matrix = torch.zeros((B, 1, 1, L), dtype=dtype, device=key_padding.device)
